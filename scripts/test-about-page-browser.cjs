...
(async () => {
  ...
  try {
    for (const width of [390, 1440]) {
      ...
      assert.ok(await page.getByRole("heading", { name: /Locally owned.*Family operated.*Community focused/i }).isVisible());
      ...
      for (const copy of [/Stacee and KC Campbell/, /June 27, 2026/, /JT and Daniel/, /official taste tester/, /Through different owners and different chapters/]) assert.ok(await page.getByRole("main").getByText(copy).isVisible());
