...
for (const token of ["business.address.street", "business.address.cityStateZip", "business.phone.href", "business.hours", "business.directionsUrl"]) assert.ok(page.includes(token), token);
assert.match(page, /aria-label="View Lutz Scoops on Google Maps"/);
